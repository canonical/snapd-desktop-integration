...
static gchar *get_json(SoupMessageBody *message_body)
{
  ...
  if (!json_parser_load_from_data (parser, message_body->data, message_body->length, NULL)) {
    return NULL;
  }
...
static void
set_setting(const gchar *schema_name, const gchar *key_name, const gchar *value)
{
  g_autofree gchar *filename = g_build_filename(temp_dir, "glib-2.0", "settings", "keyfile", NULL);
  g_autoptr(GSettingsBackend) backend = g_keyfile_settings_backend_new(filename, "/", NULL);
  g_autoptr(GSettings) settings = g_settings_new_with_backend(schema_name, backend);
  g_settings_set_string(settings, key_name, value);
}

static void
send_snapd_response(SoupMessage *message, guint status_code, const gchar *json)
{
  ...
  soup_message_headers_set_content_length(message->response_headers, strlen(json));
  soup_message_body_append(message->response_body, SOUP_MEMORY_COPY, json, strlen(json));
}

static void
handle_snapd_themes_request(SoupMessage *message)
{
  ...
  case STATE_GET_EXISTING_THEME_STATUS:
    ...
    g_assert_cmpstr(query, ==, "gtk-theme=GtkTheme1&icon-theme=IconTheme1&icon-theme=CursorTheme1&sound-theme=SoundTheme1");
    send_snapd_response(message, 200, "{\"type\":\"sync\",\"status-code\":200,\"status\":\"OK\",\"result\":{\"gtk-themes\":{\"GtkTheme1\":\"installed\"},\"icon-themes\":{\"IconTheme1\":\"installed\",\"CursorTheme1\":\"installed\"},\"sound-themes\":{\"SoundTheme1\":\"installed\"}}}");
  ...
  case STATE_GET_NEW_THEME_STATUS:
    g_assert_cmpstr(message->method, ==, "GET");
    g_assert_cmpstr(query, ==, "gtk-theme=GtkTheme2&icon-theme=IconTheme2&icon-theme=CursorTheme2&sound-theme=SoundTheme2");
    send_snapd_response(message, 200, "{\"type\":\"sync\",\"status-code\":200,\"status\":\"OK\",\"result\":{\"gtk-themes\":{\"GtkTheme2\":\"available\"},\"icon-themes\":{\"IconTheme2\":\"installed\",\"CursorTheme2\":\"unavailable\"},\"sound-themes\":{\"SoundTheme2\":\"available\"}}}");
  ...
  case STATE_INSTALL_THEMES:
    ...
    g_assert_cmpstr(json, ==, "{\"gtk-themes\":[\"GtkTheme2\"],\"icon-themes\":[],\"sound-themes\":[\"SoundTheme2\"]}");
    send_snapd_response(message, 200, "{\"type\":\"async\", \"change\": \"1234\"}");
...
static void
handle_snapd_get_changes_request(SoupMessage *message)
{
  send_snapd_response(message, 200, "{\"type\":\"sync\",\"status-code\":200,\"status\":\"OK\",\"result\":{\"id\":\"1234\",\"ready\":true}}");
}

static void
handle_snapd_request(SoupServer *server, SoupMessage *message, const char *path, GHashTable *query, SoupClientContext *client, gpointer user_data)
{
   if (strcmp(path, "/v2/accessories/themes") == 0) {
     handle_snapd_themes_request(message);
   } else if (strcmp(path, "/v2/changes/1234") == 0 && strcmp(message->method, "GET") == 0) {
     handle_snapd_get_changes_request(message);
   } else {
     send_snapd_response(message, 404, "{\"type\":\"error\",\"status-code\":404,\"status\":\"Not Found\",\"result\":{\"message\":\"not found\"}}");
   }
}

static gboolean
setup_mock_snapd(GError **error)
{
   ...
   snapd_server = soup_server_new(SOUP_SERVER_SERVER_HEADER, "MockSnapd/1.0", NULL);
...
static gboolean setup_session_bus(GError **error)
{
   ...
   g_autoptr(GSubprocessLauncher) launcher = g_subprocess_launcher_new(G_SUBPROCESS_FLAGS_STDOUT_SILENCE | G_SUBPROCESS_FLAGS_STDERR_SILENCE);
   g_subprocess_launcher_take_fd(launcher, address_pipe_fds[1], address_pipe_fds[1]);
   g_autofree gchar *address_fd_arg = g_strdup_printf("%d", address_pipe_fds[1]);
   dbus_subprocess = g_subprocess_launcher_spawn(launcher, error, "dbus-daemon", "--nofork", "--session", "--print-address", address_fd_arg, NULL);
...
static gboolean
launch_snapd_desktop_integration()
{
   ...
   g_subprocess_launcher_setenv(launcher, "DBUS_SESSION_BUS_ADDRESS", dbus_address, TRUE);

   g_autofree gchar *daemon_path = g_build_filename(DAEMON_BUILDDIR, "snapd-desktop-integration", NULL);
   g_autofree gchar *snapd_socket_path_arg = g_strdup_printf("--snapd-socket-path=%s", snapd_socket_path);

   g_autoptr(GError) error = NULL;
   g_autoptr(GSubprocess) subprocess = g_subprocess_launcher_spawn(launcher, &error, daemon_path, snapd_socket_path_arg, NULL);
   if (subprocess == NULL) {
      return FALSE;
   }
...
static void
notifications_name_acquired_cb (GDBusConnection *connection, const gchar *name, gpointer user_data)
{
   g_assert_true(launch_snapd_desktop_integration());
}

static void
handle_notifications_method_call(GDBusConnection *connection, const gchar *sender, const gchar *object_path, const gchar *interface_name, const gchar *method_name, GVariant *parameters, GDBusMethodInvocation *invocation, gpointer user_data)
{
   ...
   if (strcmp(method_name, "GetServerInformation") == 0) {
      g_dbus_method_invocation_return_value(invocation, g_variant_new("(ssss)", "mock-notifications-server", "Test", "1.0", "1.2"));
   } else if (strcmp(method_name, "GetCapabilities") == 0) {
      g_autoptr(GVariantBuilder) capabilities = g_variant_builder_new(G_VARIANT_TYPE("as"));
      ...
      g_variant_get(parameters, "(&su&s&s&s^asa{sv}i)", &app_name, &replaces_id, &app_icon, &summary, &body, &actions, &hints, &expire_timeout);
      ...
      if (state == STATE_PROMPT_INSTALL) {
         ...
         g_assert_true(g_dbus_connection_emit_signal(connection, NULL, "/org/freedesktop/Notifications", "org.freedesktop.Notifications", "ActionInvoked", g_variant_new("(us)", notification_id, "yes"), NULL));
...
{
   g_autoptr(GDBusConnection) connection = g_dbus_connection_new_for_address_sync(dbus_address, G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_CLIENT | G_DBUS_CONNECTION_FLAGS_MESSAGE_BUS_CONNECTION, NULL, NULL, error);
   if (connection == NULL) {
      g_prefix_error(error, "Failed to connect to session bus: ");
      return FALSE;
   }

   g_bus_own_name_on_connection(connection, "org.freedesktop.Notifications", G_BUS_NAME_OWNER_FLAGS_NONE, notifications_name_acquired_cb, NULL, NULL, NULL);
   ...
   if (g_dbus_connection_register_object(connection, "/org/freedesktop/Notifications", node_info->interfaces[0], &notifications_vtable, NULL, NULL, error) == 0) {
