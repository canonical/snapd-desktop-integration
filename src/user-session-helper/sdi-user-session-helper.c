...
#define EXIT_AND_RELOAD 2
#define EXIT_AND_DONT_RELOAD 0
...
static gboolean sdi_session_is_desktop(const gchar *object_path) {
  ...
  if ((session_type == NULL) || (*session_type == 0)) {
...
static gboolean sdi_check_graphical_sessions(Login1Manager *login_manager) {
  ...
  for (int i = 0; i < g_variant_n_children(sessions); i++) {
    ...
    g_variant_get(session, "(&su&s&s&o)", NULL, NULL, NULL, NULL, &session_object);
...
static void exit_by_signal (int v) {
  exit(2);
}
...
static void set_signal(int signum) {
  ...
  action.sa_handler = exit_by_signal;
...
int main(int argc, char **argv) {
  ...
  if (argc < 2) {
    ...
    // We are managing '1' as SUCCESS at systemd level, so returning
    // this will avoid relaunching the daemon.
    return EXIT_AND_DONT_RELOAD;
  }
  set_signal (SIGINT);
  set_signal (SIGTERM);
  set_signal (SIGHUP);
  if (!sdi_wait_for_graphical_session()) {
    ...
    // Returning an error code will ensure that the daemon is reloaded.
    // This will ensure that it will read the new environment variables.
    return EXIT_AND_RELOAD;
  }
  g_debug("Fully initialized graphical session detected. Launching daemon.");
  // when Wayland connection is lost, the application returns with 1 as error code,
  // so we must capture it and return a different one to ensure that it is reloaded.
  // Error code 1 must be only used when we don't want to reload (for example, because
  // snapd found a problem, like when launching it in GDM).
  childpid = fork();
  if (childpid == 0) {
    execve(argv[1], argv + 1, environ);
    return EXIT_AND_RELOAD; // just to avoid warnings in compiler; really does nothing.
  }
  waitpid(childpid, &retval, 0);
  g_print("Child process exited with value %d\n", retval);
  return EXIT_AND_RELOAD; // to ensure that it is reloaded.
}
