...
static void notice_cb(GObject *object, SnapdNotice *notice, gboolean first_run,
                      gpointer monitor) {
  SdiSnapdMonitor *self = SDI_SNAPD_MONITOR(monitor);
...
static void launch_snapd_monitor_after_error(gpointer data) {
  SdiSnapdMonitor *self = SDI_SNAPD_MONITOR(data);
...
void sdi_snapd_monitor_init(SdiSnapdMonitor *self) {
  configure_snapd_monitor(self);
}

void sdi_snapd_monitor_class_init(SdiSnapdMonitorClass *klass) {
